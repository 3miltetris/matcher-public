...
- **The API key's real daily quota is far below the 1,000/day this file used to claim, and the revision check has never been run against it.** On 2026-09-15 the key returned `429 {"code":"900804" … "You have exceeded your quota"}` after roughly **35–40 requests** (that day's daily run, ~20, plus ~15 diagnostic calls). Per [GSA's own guidance](https://open.gsa.gov/api/get-opportunities-public-api/), the tiers are: individual/personal key ≈ 10/day, **non-federal system account = 1,000/day**, federal system account up to 10,000/day — and only federal system accounts may request an increase. The behaviour above is an *individual* key, not a system account. **Registering a non-federal System Account in SAM.gov is the single highest-value fix for every quota problem here** — it is free, and it is what the `max_api_calls: 600` revision-check default has always silently assumed. **Status: requested 2026-09-16, awaiting GSA review** (see "SAM.gov static egress IP" for the approval checklist and the submitted IPs). Until that happens: the `revision_check` mode (never yet executed — no `mode: revision_check` status file exists in `sam-gov-jobs/`) will abort on `QuotaExhaustedError` within a minute of starting, and enabling `include_awards` on the daily schedule roughly doubles the query count.
...
| `views/contact_importer.py` | Import Contacts | Upload any lead spreadsheet **or** pull a HubSpot company list (lists search → memberships → batch company read) → map columns → dedup preview vs GCS (per-source or all-sources scope) → choose profiling method (🌐 scrape+GPT summary or 🔬 Deep Research technology focus, with model picker + cost estimate + >$50 confirmation) → stage file + trigger `contact-import-job` Cloud Run Job → poll `contact-import-jobs/{run_id}/status.json` |
| `views/client_editor.py` | Client Records | Select a company from `data/all-contacts/clients/` → edit its `summary` → re-embed (float64, matching stored dtype) → apply to all contact rows of that company → rewrite the source parquet in place. **Admins only:** a "🗑 Delete clients" section (multiselect + type-DELETE confirmation) removes companies that are no longer clients — every contact row, their aspect profile, and their Drive Sync assignment |
| `views/finance_researcher.py` | Deep Research | Select clients from `data/all-contacts/clients/` → pick research focus (💰 Financials / 🔬 Technology & R&D) → launch background Deep Research tasks (one per company) → poll `finance-research-runs/` or `tech-research-runs/` `{run_id}/state.json` → review parsed results → apply `financial_data`/`financial_summary`/`financials_updated_at` or `technology_data`/`technology_summary`/`technology_updated_at` back onto client rows |
...
| `sam_gov_api_key` | SAM.gov Upload view (API fetch tab) | `st.secrets['sam_gov_api_key']` — passed into the sam-gov-job config JSON (**not** a Secret Manager secret; it therefore sits in plaintext inside the `sam-gov-configs/daily_schedule.json` GCS blob, unlike the Fathom key which is deliberately Secret Manager-only). **Two tiers exist and they are not interchangeable:** the *individual* key (Account Details → API Key) is what the project ran on until 2026-09-15 and dies after a few dozen requests/day; a *system account* key gets 1,000/day but is created under SAM.gov Workspace → System Accounts, requires the Non-Federal System Administrator role from your Entity Administrator, **expires every 90 days** (replacement auto-generated 15 days ahead, both valid during the overlap), and is bound to the IP allowlist documented under "SAM.gov static egress IP". Rotation touches two places: `streamlit-secrets` and the `daily_schedule.json` blob. |
...
> ⏳ **PENDING GSA REVIEW (submitted 2026-09-16).** The non-federal system account request is with GSA and has not been approved yet. **The job is still running on the old *individual* API key**, which is not IP-restricted — so the NAT below changes nothing operationally until the system account key arrives. Nothing is blocked on the review except the quota ceiling.
>
...
> 2. Update it in **both** places: `sam_gov_api_key` in the `streamlit-secrets` Secret Manager secret, **and** the plaintext copy inside the `sam-gov-configs/daily_schedule.json` GCS blob. Missing the second is a silent failure — the daily Cloud Scheduler run reads only the blob.
> 3. Confirm the first run's `api_calls_used` in `sam-gov-jobs/{run_id}/status.json` is no longer truncated by quota, and that `rows_deferred_quota` is 0.
> 4. Diary the 90-day key expiry — system account keys rotate on a 90-day cycle (replacement auto-generated 15 days early, both valid during the overlap).
>
> If the request is **rejected**, the likely cause is the IP list or the ISSO contact; the addresses below are what was submitted.
...
**The addresses submitted to SAM.gov are `34.60.178.41` and `35.223.63.211`.** Both were listed on the system account request of 2026-09-16 (pending review — see the banner above); a redundant pair means NAT can fail over and more gateways can be added later without re-editing the account. **Never delete or re-create these addresses** — releasing one silently breaks every SAM.gov call from the job until the account's IP list is edited and re-reviewed by GSA.
