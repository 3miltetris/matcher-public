...
| `anthropic_utils.py` | `response_text`, `EmptyResponseError` | The one safe way to read a Claude response. **Never write `resp.content[0].text`** — it raises `IndexError` on an empty content list, which Claude returns when it declines (`stop_reason='refusal'`), and `IndexError` is not the `ValueError` that every retry/parse loop in this codebase catches, so the failure escapes its own handler and surfaces as `list index out of range` with no model, company or reason attached. It also assumes block 0 is text. `response_text()` joins every text block and raises `EmptyResponseError` (a `ValueError` subclass, so existing handlers keep working) carrying `stop_reason`. Pass `default=''` only where an empty answer is survivable and has somewhere sane to go — the single such call site is `matching_job`'s AI validation, where an exception would abort the whole run's `asyncio.gather` and `'no'` is already the defined outcome for an unscoreable pair. Stdlib-only; every job image COPYs it explicitly except matching-job, whose Dockerfile takes all of `src/`. |
...
**A sibling failure class, same shape:** `resp.content[0].text`. See `src/modules/anthropic_utils.py` — a refusal returns zero content blocks, `IndexError` is not `ValueError`, and the retry loop that exists to handle bad responses never sees it. Every live call site now goes through `au.response_text()`; the two unimported legacy modules (`src/llm_functions.py`, `src/modules/ai_analyzer.py`) were converted too so reviving them is not a fresh trap, though they still pass `temperature` and would fail on that first.
