...
def render():
    ...
    with col_reload:
        if st.button('↺ Reload', help='Refresh resume data from GCS'):
            st.session_state.rs_resumes = None
            st.rerun()

    # Cached in session state: every widget interaction on this page triggers a
    # rerun, and without this the whole data/resumes/ prefix was re-downloaded
    # from GCS on each one — including on every keystroke in the query box.
    # Reload above is what re-reads it. Matches how the other views cache.
    if st.session_state.get('rs_resumes') is None:
        with st.spinner('Loading resumes from GCS…'):
            st.session_state.rs_resumes = _load_resumes()
    resumes_df, load_errors = st.session_state.rs_resumes
