...
def _write_sam_config(client: storage.Client, config: dict) -> str:
    return uc.write_job_config(client, _SAM_CFG_PREFIX, config)


def _trigger_sam_job(config_blob_path: str) -> None:
    uc.trigger_job(_get_credentials(), _JOB_NAME, config_blob_path)


def _poll_sam_status(client: storage.Client, run_id: str) -> dict | None:
    return uc.poll_status(client, 'sam-gov-jobs/', run_id)
...
def _detect_col(columns: list[str], field: str) -> str | None:
    # .strip() matters: SAM.gov exports frequently carry trailing spaces in
    # header cells, and without it auto-detection silently finds nothing and
    # the user has to map every column by hand. Matches contact_importer and
    # resume_importer, which have always stripped.
...
def render():
        ...
        if not _sam_key:
            ...
            with _dc_r:
                _d_fetch_desc = st.checkbox(
                    ...
                    key='sam_daily_fetch_desc',
                )

            if st.button('💾 Save Daily Schedule', key='sam_save_daily_btn'):
                _new_daily = {
                    'run_id':      'daily',
                    'input_mode':  'api',
                    'api_params':  {
    ...
    with st.expander('🔁 Revision Check', expanded=False):
        ...
        if not _rc_key:
            ...
            if st.button('🔍 Check stored notices for revisions', type='primary', key='sam_rc_run_btn'):
    ...
    with st.expander('➕ Custom columns', expanded=bool(st.session_state.sam_custom_cols)):
        ...
        with sna1:
            new_sc_name = st.text_input(
                'Column name', placeholder='e.g. campaign_name',
                key='sam_new_sc_name', label_visibility='collapsed',
            )
        with sna2:
            new_sc_val = st.text_input(
                'Value', placeholder='e.g. Spring 2026',
                key='sam_new_sc_val', label_visibility='collapsed',
            )
        with sna3:
            if st.button('Add', key='sam_add_sc_btn', use_container_width=True):
